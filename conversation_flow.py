...
"""
Flujo V2 FINAL - DEPLOY $49 - 5 preguntas + fix loop + botones Si/No otra carta + payment_url unificado
"""
...
async def handle_incoming_text(db: AsyncSession, phone: str, text: str):
    ...
    if step == "AWAITING_Q5":
        ...
        await wa.send_text(
            ...
            "Voy a generar tu carta astral completa, tu afinidad, tus 5 zodiacos y hare la tirada de tarot "
            ...
            try:
                ...
                if chart and chart.payment_status == "pending":
                    ...
                    await wa.send_buttons(
                        ...
                        "Que deseas hacer?",
                        buttons=[
                            {"id": "reenviar_link", "title": "Reenviar link de pago"},
                            {"id": "cancelar_compra", "title": "Cancelar lectura"},
                        ]
                    )
                    return
            except Exception:
    ...
    # COMPLETED - CIERRE CON PREGUNTA Y BOTONES SI/NO
    if step == "COMPLETED":
...
async def _generate_and_prepare_payment(db: AsyncSession, state: ConversationState, phone: str):
    ...
    try:
        # Carta sin tarot para evitar truncado - tarot va aparte
        if birth_time:
        ...
        if questions:
            tarot = await astro.generate_tarot_reading(questions, lang=data.get("lang", "es"))
